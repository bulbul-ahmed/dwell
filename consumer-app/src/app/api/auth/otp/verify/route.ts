...
export async function POST(request: NextRequest) {
  const { email, code, name, role } = await request.json() as { email: string; code: string; name?: string; role?: string };
  ...
  if (!user) {
    ...
    [user] = await db.insert(users)
      .values({ name: name ?? normalEmail.split('@')[0], email: normalEmail, role: dbRole })
      .returning();
  }
